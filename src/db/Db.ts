...
export async function readEntries(
  sort: ViewSort = ViewSort.BY_PRIORITY,
  showDone: boolean,
): Promise<Entry[]> {
  ...
  const ordering =
    sort === ViewSort.BY_PRIORITY
      ? "priority"
      : `IFNULL(deadline, ${YEAR_2100_IN_MSEC})`;
...
export async function replaceDb(wireDb: WireDbFull): Promise<void> {
  ...
  await db.run(
    "INSERT INTO meta (id, epochUpdateMsecs) VALUES (1, ?)",
    wireDb.epochUpdateMsecs,
  );

  for (const entry of Object.values(wireDb.entries)) {
    await db.run(
      "INSERT INTO tasks (id, fulltext, hash, epochUpdateMsecs) VALUES (?,?,?,?)",
      [entry.id, entry.fulltext, entry.hash, entry.epochUpdateMsecs],
    );
  }
}
...
export async function upsertTodo(
  ...
  const existing = await db.all(
    "SELECT id, hash, epochUpdateMsecs, fulltext FROM tasks WHERE id=?",
    entry.id,
  );
  ...
  await db.run(
    "INSERT OR REPLACE INTO tasks (id, hash, epochUpdateMsecs, fulltext) VALUES (?,?,?,?)",
    [entry.id, entry.hash, timestampMsecs, entry.fulltext],
  );
  await db.run(
    "INSERT OR REPLACE INTO meta (id, epochUpdateMsecs) VALUES (1, ?)",
    [timestampMsecs],
  );
...
export async function replaceEntries(entries: Entry[]): Promise<void> {
  ...
  for (const entry of entries) {
    const deadline = entry.summary.deadline
      ? entry.summary.deadline.getTime()
      : "NULL";
    await db.run(
      "INSERT INTO tasks (id, done, priority, deadline, fulltext) VALUES (?,?,?,?,?)",
      [
        entry.summary.id,
...
async function getPriority(db: sqlite.Database, priority: Priority) {
  try {
    return (
      await db.get(
        `SELECT priority FROM tasks ORDER BY priority ${priority === Priority.TOP ? "ASC" : "DESC"} LIMIT 1`,
      )
    ).priority;
  } catch (e) {
...
export async function topqueue(entryId: string): Promise<void> {
  ...
  const entryText = (
    await db.get("SELECT fulltext FROM tasks WHERE id=?", [entryId])
  ).fulltext;
  ...
  await db.run("UPDATE tasks SET priority=?, fulltext=? WHERE ID=?", [
    topPriority - 1,
    entry.fulltext,
    entryId,
  ]);
...
function swapAndCollapsePriorities(
  first: number,
  second: number,
): [number, number] {
  if (first < 0 && second > 0) {
...
export async function bumpPriority(
  entryId: string,
  direction: PriorityBump,
): Promise<void> {
  const db = await connection();
  const entryPriority = (
    await db.get("SELECT priority FROM tasks WHERE id=?", [entryId])
  ).priority;

  const tweakDirection = direction == PriorityBump.LOWER ? "ASC" : "DESC";
  const tweakComparison = direction == PriorityBump.LOWER ? ">=" : "<=";

  const entryAndNeighbor = await db.all(
    `SELECT id, priority, fulltext FROM tasks WHERE priority ${tweakComparison} ? ORDER BY priority ${tweakDirection} LIMIT 2`,
    [entryPriority],
  );
  ...
  if (entryAndNeighbor.length !== 2) {
    throw Error(
      `Expected 2 values for entry and neighbor, saw ${entryAndNeighbor.length}`,
    );
  }

  const [entry, neighbor] = entryAndNeighbor;

  const [newEntryPriority, newNeighborPriority] = swapAndCollapsePriorities(
    entry.priority,
    neighbor.priority,
  );
...
export async function setTodoStatus(
  ...
  await db.run("UPDATE tasks SET done=?, fulltext=? WHERE ID=?", [
