...
function localStoreToEntries(
  storeEntries: WireEntry[],
  sortBy: SortColumn,
  hideDone: boolean,
): Entry[] {
  let entries = storeEntries.map((entry) =>
...
export const App = () => {
  ...
  const entries = localStoreToEntries(
    Object.values(localStore.store.entries),
    sortBy,
    hideDone,
  );
  ...
  const onSetPriority = useCallback(
    (id: string, operation: PriorityOperations) => {
  ...
  const onNewTask = useCallback(
    (headlineText: string, bodyText: string) => {
      const entry = parseEntry(
        fulltextToLines(`* ${headlineText}\n${bodyText}`),
      );
  ...
  const onReplaceBody = useCallback(
    (entry: Entry, newBody: string) => {
  ...
  const onClickShowOrgImporter = useCallback(
    () => setShowOrgImporter(!showOrgImporter),
    [showOrgImporter, setShowOrgImporter],
  );
  const onClickShowNewTask = useCallback(
  ...
  return (
    <div>
      ...
      <div>
        <button onClick={onToggleSortBy}>Sorting by {sortBy}</button>
      </div>
      <div>
        <input
          type="checkbox"
              ...
              <div className="chip">
                ...
                <div onClick={() => setDetailEntryId(entry.summary.id)}>
                  ...
                  <div className="fulltext">
                    <pre>{entry.summary.body}</pre>
                  </div>
                </div>
                <div className="buttonPanel">
                  <button
                    onClick={() => onSetPriority(entry.summary.id, "topqueue")}
                  >
                    &#x219F;
                  </button>
                  <button
                    onClick={() => onSetPriority(entry.summary.id, "up1")}
                  >
                    &#x2191;
                  </button>
                  <button
                    onClick={() => onSetPriority(entry.summary.id, "down1")}
                  >
                    &#x2193;
                  </button>
                  <button
                    onClick={() => onSetPriority(entry.summary.id, "bury")}
                  >
                    &#x21A1;
                  </button>
                </div>
