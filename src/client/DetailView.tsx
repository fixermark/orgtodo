...
export const DetailView: React.FC<DetailViewProps> = ({entry, onToggleTodo, onReplaceBody, onDismiss}) => {
  const onSetCheckLine = useCallback((bodyLine: number, status: CheckboxStatus) => {
  ...
  return (
	...
	<TodoLine entry={entry} onToggleTodo={onToggleTodo} />
        ...
        <DetailBody
	  entry={entry}
	  onSetCheckLine={(line: number, status: CheckboxStatus) => onSetCheckLine(line, status)} />
      </div>
    </div>
  );
};
